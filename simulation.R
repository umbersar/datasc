...
a1=replicate(n=3,rnorm(25,5,2))
a2=replicate(n=3,rnorm(100,5,2))
a3=replicate(n=3,rnorm(10000,5,2))

apply((a1),MARGIN = 2, hist)

apply(a1,MARGIN = 2, mean)
apply(a2,MARGIN = 2, mean)
apply(a3,MARGIN = 2, mean)

sd(apply(a1,MARGIN = 2, mean))
sd(apply(a2,MARGIN = 2, mean))
sd(apply(a3,MARGIN = 2, mean))


sd(apply(a1,MARGIN = 2, sd))
sd(apply(a2,MARGIN = 2, sd))
sd(apply(a3,MARGIN = 2, sd))
...
x<-seq(2,20)
y_fixed <- a+ b*x
plot(y_fixed~x)#plot(x,y_fixed)
abline(a,b)

#generate synthetic simulated gaussian/normal data
y.sim.1 <- rnorm(length(x), mean = y_fixed, sd=2)
plot(y.sim.1~x)
abline(a,b)

#now do linear regression on synthetic data
y.sim.1.lm <- lm(y.sim.1~x)
summary(y.sim.1.lm)
confint(y.sim.1.lm)
#this draws the linear regression line on the sythetic data
abline(reg = y.sim.1.lm, lty=2)
...
anova(lm(y.sim.1 ~ x))
anova(lm(y.sim.1 ~ x))[2,2]
sum(resid(lm(y.sim.1 ~ x))^2)
