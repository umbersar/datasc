...
# Compute and display initial cost and gradient/slope
costObj <- J(X, y, initial_theta) 
grd <- grad(X,y, initial_theta)

cat(sprintf('Cost at initial theta (zeros): %f\n', costObj))#cost J = 0.6931472
cat(sprintf('Gradient at initial theta (zeros): \n'))
cat(sprintf(' %f \n', grd))#gradients:  -0.100000 -12.00922 -11.26284
...
optimRes <- optim(par = initial_theta, fn = J_for_optim(X,y), gr = grad_for_optim(X,y), 
...
cat(sprintf('theta: \n'))
cat(sprintf(' %f \n', theta))#-25.0894917   0.2056570   0.2008903
