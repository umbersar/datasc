...
# regression
cost <- J_regulalized(X, y, initial_theta, lambda) #costFunctionReg(X, y, lambda)(initial_theta)
grd <- grad_regulalized(X,y, initial_theta, lambda)#gradReg(X,y, lambda)(initial_theta)
cat(sprintf('Cost at initial theta (zeros): %f\n', cost))#0.693
cat(sprintf(' %f \n', grd))#first five gradients: 0.008475 0.018788 0.000078 0.050345 0.011501 
...
# Initialize fitting parameters. this time use theta 1
initial_theta <- rep(1,dim(X)[2])
...
optimRes <- optim(par = initial_theta, 
                  fn = J_regulalized_for_optim(X,y,lambda), 
                  gr = grad_regulalized_for_optim(X,y,lambda), 
                  method="BFGS", 
