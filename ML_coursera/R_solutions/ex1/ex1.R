...
X <- cbind(rep(1,m),X) # Add a column of ones to x as the dummy feature 
X <- as.matrix(X)

# initialize the starting theta(the random location on the hill from where the 
...
theta <- gd$theta#-3.629812  1.166314
